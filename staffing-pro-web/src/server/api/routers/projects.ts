...
export const projectsRouter = createTRPCRouter({
  projectCount: protectedProcedure.query(async ({ ctx }) => {
    try {
      const response = await fetch(`${env.MAIN_API_URL}/v1/projects/count`, {
        method: 'get',
        headers: {
      ...
      const responseBody = (await response.json()) as Count
      return responseBody
    } catch (err) {
